...
function open_detail(bubbleID, factID) {
  ...
  detailElement
    ...
    .setAttribute("src", "Data/qrcodes/" + factID + ".png");
  ...
  if (direction == "right") {
    ...
    if (moves_count % parseInt(speed) == 0) {
      ...
      bubbleElement.style.top =
        parseInt(path[moves_count]) * (1 + Math.cos(angle)) + "px";
    }
  ...
  fetch("Data/seer.json").then((defitions) => {
    defitions.json().then((questions) => {
      ...
      questions.forEach((question) => {
        let newBubbleHTML = document
          .querySelector("#bubble-template")
          .content.cloneNode(true);

        const bubble_id = "bubble_" + index;

        newBubbleHTML.querySelector("#question-text").innerHTML =
          question["question"] + "?";
        newBubbleHTML
          .querySelector("#question-text")
          .setAttribute("onclick", "open_question('" + bubble_id + "')");
        newBubbleHTML
          .querySelector("#close-grid-btn")
          .setAttribute("onclick", "close_question('" + bubble_id + "')");
        ...
        for (let i = 0; i < 6; i++) {
          if (question["cards"][i] !== undefined) {
            ...
            fact.querySelector("#number").innerHTML =
              question["cards"][i]["number"];
            if (parseInt(question["cards"][i]["number"].length) < 8) {
              fact
                .querySelector("#number")
                .classList.remove("open-question-number");
              fact
                .querySelector("#number")
                .classList.add("open-question-number-2");
            }

            fact.querySelector("#text").innerHTML =
              question["cards"][i]["phrase"];
        ...
        newBubbleHTML
          ...
          .setAttribute("onclick", "close_detail('" + bubble_id + "')");
